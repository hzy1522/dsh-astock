...
A股量化工作台 —— DeepSeek Harness 插件。

在侧边栏底部提供独立的「A股量化」整页：自选股管理、K线图、公司财务数据、策略配置与回测。
...
| 策略 | 6 个参数化模板（双均线、MACD、RSI、布林带、唐奇安突破、均线多头）+ 表达式编辑器 + JavaScript 编辑器 + **用文字描述让 AI 生成** |
| 回测 | **按市场自动切换规则**（A 股 T+1/涨跌停/100 股一手；港股 T+0/无涨跌停/每手逐股不同/印花税双边）；佣金(5元起)/印花税/其他费率/滑点；总收益/年化/最大回撤/夏普/卡玛/胜率/盈亏比；资金曲线 + 沪深300 对比；交易明细 |
...
node tests/host.test.mjs     # Host 半边：真实上游 + mock llm + 港股 + 自选股混排（79 项）
node tests/client.test.mjs   # 客户端 bundle：真实执行组件 + 数据流 + 策略模式 + AI 入口 + 港股规则 + 回测区间与档位（100 项）
```

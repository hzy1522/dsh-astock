...
lib/index.js             Host 半边：8 条 /astock/api/* 路由 + 自选股与免责状态落盘
...
node tests/host.test.mjs     # Host 半边：真实上游 + mock llm + 港股 + 自选股混排 + 免责声明 + AI 生成诊断（106 项）
node tests/client.test.mjs   # 客户端 bundle：真实执行组件 + 数据流 + 策略模式 + AI 入口 + 港股规则 + 回测区间与档位 + 免责弹窗 + 情绪因子 + 交易证据链与一键改写（164 项）
```
...
测试：client 139 → 164 项（新增证据链、`AND`/`OR`/`NOT` 节点标注、实际触发项标注、`OR` 假分支不误标、
原生比较的兜底说明、一键改写与换回），host 103 → 106 项（AI 提示词必须要求条件函数与 `why`）。合计 313 项全通过。

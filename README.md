...
**当前版本 `0.5.0`** · [npm](https://www.npmjs.com/package/dsh-astock) · [版本记录](#版本记录) · [提交历史](https://github.com/hzy1522/dsh-astock/commits/main)
...
| 公司数据 | 实时快照（现价/涨跌/开高低/成交额/换手/PE/PB/总市值/流通市值/**每手股数**/**币种**）+ 16 期财务（EPS/BPS/ROE/毛利率，仅 A 股）+ **公司动态**（真实事件日期：说明会 / 财报预约披露 / 除权除息） |
| 策略 | 9 个参数化模板（双均线、MACD、RSI、布林带、唐奇安突破、均线多头、**相对强弱 RS**、**大盘趋势+相对强弱**、**说明会前后**）+ 表达式编辑器 + JavaScript 编辑器 + **用文字描述让 AI 生成** |
...
- **把真实事件日期一并交给模型**。你自己算不出「哪天开会」「那天是不是交易日」，模型同样算不出，所以宿主会先把该股的真实事件列表（见[因子](#已接入公司事件可回测)）塞进提示词，并明确要求：只能用这些日期、禁止自己推算日历、禁止写死日期字符串。比如直接写：
...
> 港股公告的数据源与 A 股不同，事件目前可能为空（界面会说明），此时用到事件因子的策略会
> 直接报错，而不是静默算出「没有信号」。
...
lib/index.js             Host 半边：9 条 /astock/api/* 路由 + 自选股/免责状态落盘 + 公司事件抓取
...
node tests/host.test.mjs     # Host 半边：真实上游 + mock llm + 港股 + 自选股混排 + 免责声明 + 事件路由 + AI 生成诊断（127 项）
node tests/client.test.mjs   # 客户端 bundle：真实执行组件 + 数据流 + 策略模式 + AI 入口 + 港股规则 + 回测区间与档位 + 免责弹窗 + 情绪因子 + 事件因子 + 交易证据链与一键改写（181 项）
```

...
| 回测 | **按市场自动切换规则**（A 股 T+1/涨跌停/100 股一手；港股 T+0/无涨跌停/每手逐股不同/印花税双边）；佣金(5元起)/印花税/其他费率/滑点；总收益/年化/最大回撤/夏普/卡玛/胜率/盈亏比；资金曲线 + 沪深300 对比；交易明细；支持**按日期区间回测**与年份快捷档位 | ; 交易明细可点击展开「为什么买/为什么卖」的证据
...
点击交易明细里的任意一行，会在下方展开这笔交易的**决策证据**：

```
▾ 2026-07-15   1204.86   2026-08-20   1298.50   +7.66%   26
    为什么买
      信号日 2026-07-14（收盘） → 成交日 2026-07-15（开盘 @1204.86）
      信号日各条件的取值：
      ✓  CROSS(MA(5), MA(20))      1202.47  vs  1197.29
      ✗  RSI(14) < 0               41.8833  vs  0.0000
    为什么卖
      信号日 2026-08-19（收盘） → 成交日 2026-08-20（开盘 @1298.50）
      …
```
...
| 模式 | 能给出的解释 |
| --- | --- |
| **表达式** | **逐条件拆解**：把顶层的 `AND` / `OR` 摊平成一条条条件，各自标注 ✓/✗ 并附上比较两侧的具体数值。**不成立的条件也会如实列出**，不会只挑成立的那条说。 |
| **JavaScript** | 任意代码（循环、状态机）**无法自动定位到具体是哪一句触发**。界面会如实说明这一点，并改为给出信号日的**指标快照**（C / MA5 / MA20 / MA60 / RSI14 / MACD / 有指数时的 RS、IDXDEV），供你对照代码自行核对。 |

这个区别是本质的，不是偷懒：JS 策略可以有任意分支与状态，静态分析给不出可信的归因。**与其编一个看起来像原因的理由，不如说清「这里只能给快照」。**
...
node tests/client.test.mjs   # 客户端 bundle：真实执行组件 + 数据流 + 策略模式 + AI 入口 + 港股规则 + 回测区间与档位 + 免责弹窗 + 情绪因子 + 交易解释（139 项）
```

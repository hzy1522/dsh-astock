...
const STRATEGY_SYSTEM_PROMPT = [
  '你是 A 股量化策略代码生成器。只输出 JavaScript 代码本身，不要解释、不要 Markdown 代码块标记。',
  '',
...
async function generateStrategy(ctx, description, currentCode) {
  ...
  if (selection && selection.reasoningEffort !== undefined) options.reasoningEffort = selection.reasoningEffort
  ...
  for await (const chunk of llm.stream(options)) {
    ...
    else if (chunk.type === 'finish' && chunk.reason !== undefined && chunk.reason.kind === 'error') {
  ...
  if (code === '') throw new Error('模型没有返回任何内容')
  return {
    code,
    provider,
    model,

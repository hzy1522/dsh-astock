...
/** 只有这类标题才值得去正文里找会议日期。 */
const MEETING_TITLE = /(关于召开|關於召開|召开|召開|会议通知|會議通知|说明会|說明會|路演)/
/** 这些是会后或变更文件，正文里的日期不是（或不再是）会议召开时间。 */
const MEETING_EXCLUDE = /(决议|決議|法律意见|法律意見|律师|律師|提示性|延期|取消|更正)/
/** 会议召开时间通常这么写；按优先级试，避免抓到正文里别的日期（股权登记日之类）。 */
const MEETING_DATE_RES = [
  /会议召开时间[^0-9]{0,12}(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日/,
  /(?:将于|定于|拟于|计划于)[^0-9\n]{0,24}(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日/,
  /(?:现场|网络|本次)?会议?(?:召开)?时间[^0-9]{0,12}(\d{4})\s*年\s*(\d{1,2})\s*月\s*(\d{1,2})\s*日/,
]
...
function meetingDateOf(content, announcedAt) {
  ...
  for (const re of MEETING_DATE_RES) {
    ...
    const iso = isoDate(found[1], found[2], found[3])
...
/** 会议类事件：逐条去公告正文里找「会议召开时间」。 */
async function loadMeetingEvents(code) {
  const announcements = await loadAnnouncements(code, 100)
  const hits = announcements
    .filter((it) => MEETING_TITLE.test(it.title) && !MEETING_EXCLUDE.test(it.title))
    .slice(0, 8)
  const bodies = await Promise.all(hits.map((it) => getJson(EM_ANNOUNCE_BODY + '?' + enc({
    art_code: it.artCode, client_source: 'web', page_index: '1',
  })).catch(() => null)))
  const out = []
  for (let i = 0; i < hits.length; i++) {
    const body = bodies[i]
    const content = body && body.data ? body.data.notice_content : ''
    const date = meetingDateOf(content, hits[i].announcedAt)
    if (date === null) continue
    out.push({
      date, kind: 'meeting', title: hits[i].title, announcedAt: hits[i].announcedAt,
      actual: null, source: '公告正文「会议召开时间」',
    })
  }
  return out
}
...
async function loadEvents(code) {
  ...
  // 三类事件各自尽力：某一路失败不该让整页事件都没了。
  const [meetings, reports, exdivs] = await Promise.all([
    loadMeetingEvents(code).catch(() => []),
    loadReportEvents(code).catch(() => []),
    loadExDivEvents(code).catch(() => []),
  ])
  const seen = new Set()
  const events = []
  for (const ev of meetings.concat(reports, exdivs)) {
  ...
  const counts = { meeting: 0, report: 0, exdiv: 0 }
  for (const ev of events) counts[ev.kind] += 1
  const value = { code, market: marketOf(code), events, counts, fetchedAt: new Date().toISOString() }
  eventCache.set(code, { at: Date.now(), value })
  return value
}
...
const STRATEGY_SYSTEM_PROMPT = [
  ...
  '- EV(n) EVMEET(n) EVREP(n) EVDIV(n) 事件因子：相对真实事件（说明会 / 财报预约披露 / 除权除息）',
  '  的第 n 个交易日为 1，其余为 0；n 可以为负（-1 = 事件前一个交易日），0 = 事件当日。',
  '  事件日期与交易日换算都由系统提供，你只写 n。',
  '',
  '硬性要求：',
  ...
  route('/astock/api/generate-strategy', async (_query, body) => {
    ...
    if (code !== '') {
      try {
        ...
        eventContext = eventBrief(payload, String((body && body.name) || ''))
        eventCount = payload.events.length
      } catch { /* 事件取不到不影响生成，只是模型看不到真实日期 */ }
    }

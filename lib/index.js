...
async function loadEvents(code) {
  ...
  const [meetings, reports, exdivsRaw] = await Promise.all([
    settle('公告事件', loadAnnouncementEvents(code)),
    settle('财报预约披露', loadReportEvents(code)),
    settle('除权除息', loadExDivEvents(code)),
  ])
  // 东财那份除权除息表偶尔为空；配了同花顺 Key 就用官方复权事件兜底。
  let exdivs = exdivsRaw
  if (exdivs.length === 0 && readHithink().key !== '') {
    exdivs = await settle('同花顺除权除息', thsExDivEvents(code))
  }
...
// ---------------- 同花顺金融数据（官方 REST API） ----------------
//
// 同花顺官方确实提供了 MCP（托管 HTTP 端点 https://fuyao.aicubes.cn/mcp/*），
// 但插件这边调不了 MCP 协议、也拿不到宿主的 tools.execute（见下面的实测结论）。
// 好在这套服务同时提供**零依赖 REST API**，用的是同一个 API Key、同一批数据，
// 于是插件直接走 REST——用户只要去 fuyao.aicubes.cn/admin 签一个 Key 填进来即可。
//
// 端点与字段来自官方契约：https://fuyao.aicubes.cn/llms-full.txt

const HITHINK_BASE = 'https://fuyao.aicubes.cn'
const HITHINK_FILE = 'hithink.json'
/** MCP 端点（供界面展示；插件自身走 REST，不连这些地址）。 */
const HITHINK_MCP = [
...
function readHithink() {
  const all = readState(HITHINK_FILE)
  const key = all && typeof all.key === 'string' ? all.key.trim() : ''
  return { key }
}

function writeHithink(key) {
  const clean = String(key || '').trim().slice(0, 200)
  writeState(HITHINK_FILE, { key: clean, savedAt: new Date().toISOString() })
  return readHithink()
}
...
/**
 * 把六位 A 股代码转成同花顺的 thscode（交易所后缀）。
 * 港股不在当前服务范围内，返回 null。
 */
function thsCode(code) {
  const text = String(code || '').trim()
  if (!/^\d{6}$/.test(text)) return null
  if (/^(6|9)/.test(text)) return text + '.SH'
  if (/^(0|3)/.test(text)) return text + '.SZ'
...
/**
 * 调一次同花顺 REST 接口。
 * @param path - 形如 /api/a-share/prices/snapshot。
 * @param params - query 参数。
 * @returns 信封里的 data。
 */
async function thsGet(path, params) {
  const key = readHithink().key
  if (key === '') throw new Error('还没有配置同花顺 API Key（去 fuyao.aicubes.cn/admin 签一个）')
  const query = enc(Object.assign({}, params || {}))
  const text = await fetchText(HITHINK_BASE + path + (query === '' ? '' : '?' + query), { 'X-api-key': key })
  let json = null
  try {
    json = JSON.parse(text)
  } catch {
    throw new Error('同花顺返回了非 JSON 内容：' + String(text).slice(0, 120))
  }
  if (json && typeof json.code === 'number' && json.code !== 0) {
    throw new Error('同花顺接口报错（code ' + json.code + '）：' + String(json.message || ''))
  }
  return json ? json.data : null
}

/** 复权事件（分红 / 送股 / 配股，含除权除息日）→ 事件表用的条目。 */
async function thsExDivEvents(code) {
  const thscode = thsCode(code)
  if (thscode === null) return []
  const data = await thsGet('/api/a-share/corporate-actions/adjustment-factors', { thscode })
  const items = data && Array.isArray(data.item) ? data.item : []
  const out = []
  for (const row of items) {
    if (row === null || typeof row !== 'object') continue
    const ms = typeof row.ex_date_ms === 'number' ? row.ex_date_ms : null
    if (ms === null) continue
    const date = new Date(ms).toISOString().slice(0, 10)
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) continue
    const bits = []
    if (typeof row.cash_dividend === 'number' && row.cash_dividend !== 0) bits.push('现金分红 ' + row.cash_dividend)
    if (typeof row.bonus_ratio === 'number' && row.bonus_ratio !== 0) bits.push('送股 ' + row.bonus_ratio)
    if (typeof row.rights_ratio === 'number' && row.rights_ratio !== 0) bits.push('配股 ' + row.rights_ratio)
    out.push({
      date,
      kind: 'exdiv',
      title: bits.length === 0 ? '除权除息日' : ('除权除息（' + bits.join('，') + '）'),
      // 同花顺给的是事件本身，公告日在别的表里；没有就按「事先已知」处理并由界面标注。
      announcedAt: null,
      actual: null,
      source: '同花顺复权事件',
    })
...
/** 供公司数据 AI 使用的同花顺工具（只有配了 Key 才会交给模型）。 */
function hithinkTools() {
  return [
    {
      name: 'ths_corporate_actions',
      description: '同花顺：查 A 股的除权除息 / 分红 / 送股 / 配股事件（含除权除息日）。参数 code 为六位股票代码。',
      parameters: { type: 'object', properties: { code: { type: 'string', description: '六位 A 股代码，如 600519' } }, required: ['code'], additionalProperties: false },
    },
    {
      name: 'ths_prices_snapshot',
      description: '同花顺：查 A 股最新行情快照（价格、涨跌幅、成交额等）。',
      parameters: { type: 'object', properties: { code: { type: 'string', description: '六位 A 股代码' } }, required: ['code'], additionalProperties: false },
    },
    {
      name: 'ths_hot_stocks',
      description: '同花顺：查当前 A 股热股榜 Top30（市场热度）。无参数。',
      parameters: { type: 'object', properties: {}, additionalProperties: false },
    },
    {
      name: 'ths_dragon_tiger',
      description: '同花顺：查龙虎榜（当日机构与游资买卖）。无参数。',
      parameters: { type: 'object', properties: {}, additionalProperties: false },
    },
  ]
}
...
/** 执行一个同花顺工具。 */
async function runHithinkTool(call) {
  const args = parseArgs(call.args)
  const code = String(args.code || '')
  try {
    if (call.name === 'ths_corporate_actions') {
      const rows = await thsExDivEvents(code)
      return { text: rows.length === 0 ? '（没有查到除权除息事件）' : rows.map((r) => '- ' + r.date + '  ' + r.title).join('\n'), isError: false }
    }
    if (call.name === 'ths_prices_snapshot') {
      const thscode = thsCode(code)
      if (thscode === null) return { text: '港股不在同花顺这套 A 股服务范围内（需要六位 A 股代码）', isError: true }
      const data = await thsGet('/api/a-share/prices/snapshot', { thscodes: thscode })
      return { text: JSON.stringify(data).slice(0, 2000), isError: false }
    }
    if (call.name === 'ths_hot_stocks') {
      const data = await thsGet('/api/a-share/special-data/hot-stock-list', {})
      return { text: JSON.stringify(data).slice(0, 2500), isError: false }
    }
    if (call.name === 'ths_dragon_tiger') {
      const data = await thsGet('/api/a-share/special-data/dragon-tiger-list', {})
      return { text: JSON.stringify(data).slice(0, 2500), isError: false }
    }
    return { text: '未知的同花顺工具：' + call.name, isError: true }
  } catch (error) {
    return { text: '同花顺接口调用失败：' + String((error && error.message) || error), isError: true }
  }
}
...
async function companyChat(ctx, request) {
  ...
  // 配了同花顺 Key 就把它那套官方数据（行情/财务/除权/热榜/龙虎榜）也交给模型。
  const thsEnabled = readHithink().key !== ''
  const thsSchemas = thsEnabled ? hithinkTools() : []
  const extraSchemas = thsSchemas.concat(enabledSources.map((item) => ({
  ...
  const base = {
    ...
    system: COMPANY_SYSTEM_PROMPT
      + (thsEnabled ? '\n\n【同花顺官方数据】你还能调用 ths_ 开头的工具，它们走同花顺官方 API，'
        + 'A 股行情、财务、除权除息、热榜与龙虎榜以它为准；港股不在其覆盖范围内。' : '')
      + (sourceNotes === '' ? '' : '\n\n' + sourceNotes),
    temperature: 0.2,
    maxTokens: 16384,
  }
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
    ...
    for (const call of step.calls) {
      ...
      const isThs = thsSchemas.some((item) => item.name === call.name)
      const result = isWeb
        ? await runWebTool(web, call, searchState)
        : (isThs
          ? Object.assign({ requests: 1 }, await runHithinkTool(call))
          : (source === undefined
...
export function apply(ctx) {
  ...
  // 同花顺官方数据：Key 的保存 / 状态 / 连通性测试。
  route('/astock/api/hithink', async (_query, body) => {
    if (body && typeof body.key === 'string') writeHithink(body.key)
    const { key } = readHithink()
    return {
      configured: key !== '',
      keyMasked: maskKey(key),
      mcp: HITHINK_MCP,
      adminUrl: HITHINK_BASE + '/admin/',
      docsUrl: HITHINK_BASE + '/docs/',
      note: '同花顺官方提供 MCP 与 REST 两套入口、同一个 API Key。插件侧走 REST'
        + '（插件上下文无法执行宿主注册的 MCP 工具）。港股不在其当前覆盖范围。',
    }
  })
  route('/astock/api/hithink/test', async () => {
    try {
      const data = await thsGet('/api/a-share/calendar/trading-days', {})
      const days = data && Array.isArray(data.item) ? data.item.length : 0
      return { ok: true, message: '连接正常，取到近一年 ' + days + ' 个交易日', sample: days > 0 ? String(data.item[days - 1].date || '') : '' }
    } catch (error) {
      return { ok: false, message: String((error && error.message) || error) }
    }
  })
  // 外部数据源：用户配置的 HTTP 接口 + 说明文本。
